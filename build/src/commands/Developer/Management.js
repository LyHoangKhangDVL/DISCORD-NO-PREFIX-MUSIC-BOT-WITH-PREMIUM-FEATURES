  ...
  constructor(client) {
    ...
    this.exec = async (message, args, prefix) => {
        ...
        return message?.channel?.send({
          embeds: [
            new EmbedBuilder()
              ...
              .setTitle(
                `⚙️ Management`,
              ).setDescription(`** ➡ \`${prefix}management add\`
      ...
      if (args[0].toLowerCase() === `add`) {
        ...
        if (!user || user === undefined)
          return message.reply({
            content: `❌ Please provide me a valid user 👤`,
          });
        let get = manageMent(user.id);
        if (get.MANAGE === 1)
          return message.reply({
            content: `❌ This User ${user.tag} is already in my Management List ⚙️`,
          });
        let ok = addManagement(user.id);
        if (ok === true) {
          return message.reply({
            content: `✅ Successfully **Added** ${user.tag} to my Management List ⚙️`,
          });
        } else if (ok === false) {
          return message.reply({
            content: `❌ Error while adding to management! ⚙️`,
          });
        }
      }
      if (args[0].toLowerCase() === `remove`) {
        ...
        //        if (get.MANAGE !== 1)
        //          return message.reply({
        //            content: `❌ This User ${user.tag} is not in my **Management List**`,
        //          });
        let remove = removeManagement(user.id);
        if (remove === true)
          return message.reply({
            content: `✅ Successfully **Removed** User ${user.tag} from my Management List 👤`,
          });
        else
          return message.reply({
            content: `❌ I couldn't interact with database!`,
          });
      }
      if (
        ...
        let collector = await msg.createMessageComponentCollector({
          filter: (b) => {
            if (b.user.id === message.author.id) return true;
              ...
              return b.reply({
                content: `❌ You are not the message requester`,
                ephemeral: true,
              });
          },
          time: 100000 * 5,
