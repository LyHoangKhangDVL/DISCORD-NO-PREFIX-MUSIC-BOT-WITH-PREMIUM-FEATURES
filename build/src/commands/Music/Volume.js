        ...
        this.exec = async (message, args, prefix) => {
            ...
            if (!dispatcher || !dispatcher.player)
                return message.reply({
                    embeds: [
                        this.client.utils
                            .errorEmbed()
                            .setDescription(`❌ There is no player for this guild yet`),
                    ],
                });
            if (!args[0]) {
                return message.reply({
            ...
            if (!Number(args[0]) || Number(args[0]) < 0 || Number(args[0]) > 200)
                return message.reply({
                    embeds: [
                        this.client.utils
                            .premiumEmbed(message.guildId)
                            .setDescription(`❌ Volume arguement must lie betwwen **0 to 200**`)
                    ],
                });
            ...
            if (Math.round(vol / 100) === dispatcher.player.filters.volume)
                return message.reply({
                    embeds: [
                        this.client.utils
                            .premiumEmbed(message.guildId)
                            .setDescription(`❌ Volume is Already set to **${vol}%**`),
                    ],
                });
            ...
            return message.reply({
                embeds: [
                    this.client.utils
                        .premiumEmbed(message.guildId)
                        .setDescription(`✅ Successfully **Changed** the Volume to **${args[0]}%**`)
                ],
            });
        };
    }
