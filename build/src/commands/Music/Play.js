    ...
    constructor(client) {
        ...
        this.exec = async (message, args, prefix) => {
                    ...
                    if (result.loadType === `PLAYLIST_LOADED`) {
                            ...
                            if (!message.guild.members.me
                                ...
                                return message.reply({
                                    embeds: [
                                        this.client.utils
                                            .premiumEmbed(message.guildId)
                                            .setDescription(`❌ I don't have **Connect** or **Speak** permissions in your voice channel`)
                        ...
                        return message.reply({
                            embeds: [
                                this.client.utils
                                    .premiumEmbed(message.guildId)
                                    .setDescription(`📜 Added **[${result.tracks.length}](${this.client.config.voteUrl})** Tracks from [${result.playlistInfo.name}](${this.client.config.voteUrl})`),
                            ],
                        });
                    }
                        ...
                        return message.reply({
                            embeds: [
                                this.client.utils
                                    ...
                                    .setDescription(`❌ [No results](${this.client.config.voteUrl}) found for the query provided`),
                            ],
                        });
                    }
                    else {
                ...
                if (!result.tracks.length)
                    return message.reply({
                        embeds: [
                            this.client.utils
                                .premiumEmbed(message.guildId)
                                .setDescription(`❌ [No Results](${this.client.config.voteUrl}) found for the query`)
                                .setTitle(`No Results`),
                        ],
                    });
                if (result.loadType === `PLAYLIST_LOADED`) {
            ...
            if (type === "direct") {
                ...
                if (!result.tracks?.length)
                    return message.reply({
                        embeds: [
                            this.client.utils
                                .premiumEmbed(message.guildId)
                                .setDescription(`❌ [No Results](${this.client.config.voteUrl}) found for the given query`)
                                .setTitle(`No Results`),
                        ],
                    });
                ...
                let node = this.client.shoukaku.getNode();
                let b1 = this.client.utils.button(`custom_id`, null, 2, `oras_default_search`, null, '▶');
                let b2 = this.client.utils.button(`custom_id`, null, 3, `oras_spoti_search`, null, '🪇');
                let buttons = [b1, b2];
                        ...
                        if (!result.tracks.length)
                            ...
                            return interaction.update({
                                embeds: [
                                    this.client.utils
                                        .premiumEmbed(message.guildId)
                                        .setDescription(`📜 Added [${result.tracks[0].info.title.substring(0, 35)}](${this.client.config.voteUrl}) to Queue`),
                                ],
                                components: [],
                            });
                        }
                            ...
                            if (!result?.tracks?.length)
                                return interaction.update({
                                    embeds: [
                                        this.client.utils
                                            .premiumEmbed(message.guild.id)
                                            .setDescription(`❌ [No Result](${this.client.config.voteUrl}) found for the query`),
                                    ],
                                    components: [],
                                });
                            let track = result.tracks[0];
                            ...
                            if (res.loadType === `LOAD_FAILED` ||
                                ...
                                return interaction.update({
                                    embeds: [
                                        this.client.utils
                                            .premiumEmbed(message.guild.id)
                                            .setDescription(`📜 Added [${tr.info.title.substring(0, 35)}](${this.client.config.voteUrl}) to Queue`),
                                    ],
                                    components: [],
                                });
                            }
